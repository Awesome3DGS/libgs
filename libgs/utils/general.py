# Credit: https://github.com/graphdeco-inria/gaussian-splatting/blob/main/utils/general_utils.py
...
def safe_state(silent: bool, seed: int = 0):
    old_f = sys.stdout
    ...
    class F:
        ...
        def write(self, x):
            if self.silent:
                return
            if x.endswith("\n"):
                now = str(datetime.now().strftime("%d/%m %H:%M:%S"))
                x = x.replace("\n", f" [{now}]\n")
            old_f.write(x)
