...
from libgs.data.types import BasicPointCloud
from libgs.data.types.point_cloud import BasicPointCloud
from libgs.utils.general import (
...
class GaussianModel:
    ...
    def __init__(
        ...
        self.mlp_color = nn.Sequential(
            nn.Linear(
                feat_dim + 3 + self.color_dist_dim + self.appearance_dim,
                feat_dim,
            ),
            nn.ReLU(True),
    ...
    def save_ply(self, path):
        ...
        attributes = np.concatenate(
            (anchor, normals, offset, anchor_feat, opacities, scale, rotation),
            axis=1,
        )
        ...
        if mode == "split":
            ...
            color_mlp = torch.jit.trace(
                ...
                (
                    torch.rand(
                        1,
                        self.feat_dim + 3 + self.color_dist_dim + self.appearance_dim,
                    ).cuda()
                ),
            )
