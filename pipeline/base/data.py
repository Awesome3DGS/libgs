...
class DataModule(BaseDataModule):
    ...
    def get_train_dataset(self, scale: float = 1.0) -> Dataset:
        ...
        if (current_frame := self.config.current_frame) is not None:
            train_dataset = train_dataset.get_frame_dataset(current_frame)
        if scale != 1:
    ...
    def get_test_dataset(self, scale: float = 1.0) -> Optional[Dataset]:
        ...
        if (current_frame := self.config.current_frame) is not None:
            test_dataset = test_dataset.get_frame_dataset(current_frame)
        if test_dataset:
            if scale != 1:
