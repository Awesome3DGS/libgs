...
class TrainerConfig(BaseTrainerConfig):
    cache_data_on_device: bool = False
    ...
    num_coarse_steps: int = 3000
    num_fine_steps: int = 14000


class Trainer(BaseTrainer):
    @property
    def num_steps(self) -> int:
        if self.module.stage == "coarse":
            return self.config.num_coarse_steps
        return self.config.num_fine_steps
    ...
    def training_loop(self):
        stages = ["fine"] if self.module.stage == "fine" else ["coarse", "fine"]
        start_frame = self.datamodule.config.start_frame
        end_frame = self.datamodule.config.end_frame
        coarse_frame = self.datamodule.config.coarse_frame
        for stage in stages:
            if stage == "coarse" and self.datamodule.config.coarse_frame is not None:
                self.datamodule.config.start_frame = coarse_frame
                self.datamodule.config.end_frame = coarse_frame + 1
            else:
                self.datamodule.config.start_frame = start_frame
                self.datamodule.config.end_frame = end_frame
            logging.info(f"\n==> Start {stage} stage learning ...")
            self.module.stage = stage
            self.stage_training_loop()

    def stage_training_loop(self):
        train_dataloader = self.datamodule.train_dataloader()
        logging.info(f"Number of views in training dataset: {len(train_dataloader)}")
        cache = self.config.cache_data_on_device
        move_to_device = lambda ts: [t.to(self.device, True) for t in ts]
