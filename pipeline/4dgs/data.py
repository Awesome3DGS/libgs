...
class DataConfig(BaseDataConfig):
    ...
    coarse_frame: Optional[int] = None
    batch_size: int = 4
    shuffle: bool = True
...
class DataModule(BaseDataModule):
    ...
    def get_train_dataset(self, scale: float = 1.0) -> Dataset:
        start, end = self.config.start_frame, self.config.end_frame
        train_dataset = self.scene.train_dataset
        if start is not None or end is not None:
            train_dataset = train_dataset.get_clip_dataset(start, end)
        if scale != 1:
    ...
    def get_test_dataset(self, scale: float = 1.0) -> Optional[Dataset]:
        ...
        if start is not None or end is not None:
            test_dataset = test_dataset.get_clip_dataset(start, end)
        if test_dataset:
            if scale != 1:
    ...
    def train_dataloader(
        ...
        if mock_test:
            ...
            kwargs["collate_fn"] = list
            kwargs["batch_size"] = max(self.config.batch_size, 1)
        if random:
    ...
    def val_dataloader(self, scale: float = 1.0) -> List[DataLoader]:
        ...
        if self.config.eval_train:
            datasets.append(self.train_dataloader(scale, random=False, mock_test=True))
